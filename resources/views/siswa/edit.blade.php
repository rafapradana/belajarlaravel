...
<head>
    ...
    <title>Edit Student - StudyFlow</title>
                ...
                <div class="bg-white shadow-sm rounded-lg border border-gray-200">
                    ...
                    <div class="px-6 py-4 border-b border-gray-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <h1 class="text-xl font-semibold text-gray-900">Edit Student</h1>
                                <p class="text-sm text-gray-600">Update {{ $siswa->nama }}'s information</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="text-center">
                                <div class="text-2xl font-semibold text-gray-900">{{ $siswa->nama }}</div>
                                <div class="text-xs text-gray-500">Name</div>
                            </div>
                            <div class="text-center">
                                <div class="text-2xl font-semibold text-gray-900">{{ $siswa->tb }} cm</div>
                                <div class="text-xs text-gray-500">Height</div>
                            </div>
                            <div class="text-center">
                                <div class="text-2xl font-semibold text-gray-900">{{ $siswa->bb }} kg</div>
                                <div class="text-xs text-gray-500">Weight</div>
                            </div>
                        </div>

                        @php
                            $heightInMeters = $siswa->tb / 100;
                            $currentBmi = $heightInMeters > 0 ? $siswa->bb / ($heightInMeters * $heightInMeters) : 0;
                            $bmiCategory = '';
                            $bmiColor = '';
                        ...
                        <div class="mt-4 text-center">
                            <span class="text-sm text-gray-600">Current BMI: </span>
                            <span class="font-medium {{ $bmiColor }}">{{ number_format($currentBmi, 1) }} ({{ $bmiCategory }})</span>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-6">
                        <form method="POST" action="{{ route('siswa.update', $siswa->id) }}" class="space-y-6">
                            ...
                            <div class="bg-gray-50 rounded-lg p-4">
                                <h4 class="text-sm font-medium text-gray-900 mb-2">Updated BMI Preview</h4>
                                <div id="bmiPreview" class="text-sm text-gray-600">
                                    BMI: <span class="font-medium {{ $bmiColor }}">{{ number_format($currentBmi, 1) }} ({{ $bmiCategory }})</span>
                                </div>
                            </div>
